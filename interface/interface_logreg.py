...
class CephaloPredictor(QWidget):
    def __init__(self):
        ...
        try:
            model_dict = joblib.load("interface/03_03_01_soc_multilabel_logreg.joblib")
            
            self.model = model_dict["model"]
            self.model_features = model_dict.get("feature_cols", [])
            self.model_outputs = model_dict.get("soc_cols", [])
            print("✅ Model loaded successfully with keys:", list(model_dict.keys()))
        except Exception as e:
            self.model = None
            self.model_features = []
            self.model_outputs = []
    ...
    def probability_model(self, age, sex, weight, height, meds_vector):
        """
        Use the trained MultiOutputClassifier logistic regression model
        to predict probabilities for each side effect.
        Builds a full feature vector matching the training feature columns.
        """
        if not hasattr(self, "model") or self.model is None:
            QMessageBox.warning(self, "Model Error", "No predictive model is loaded.")
            return {eff: {"prob": 0.0, "severity": "Not Severe", "color": "#e2e8f0"} for eff in SIDE_EFFECTS}

        feat_names = self.model_features
        x_dict = {f: 0.0 for f in feat_names}

        # --- 1️⃣ Fill demographics matching training names exactly ---
        ...
        for med, val in meds_vector.items():
            if val == 1:
                ...
                found = False
                for f in feat_names:
                    # robust matching (case-insensitive, ignore punctuation and plurals)
                    f_clean = f.lower().replace(",", "").replace("(", "").replace(")", "").strip()
                    m_clean = med_lower.replace(",", "").replace("(", "").replace(")", "").strip()
                    if m_clean in f_clean or f_clean in m_clean:
                        x_dict[f] = 1.0
                        found = True
                if found:
        ...
        if "N_MEDS" in feat_names:
            x_dict["N_MEDS"] = sum(meds_vector.values())

        # --- 3️⃣ Convert dict → NumPy vector in correct feature order ---
        x = np.array([[x_dict[f] for f in feat_names]])
        ...
        try:
            ...
            probs = np.array([p[0, 1] if p.ndim == 2 else p[0] for p in probs_list])
            probs = np.clip(probs * 100, 0, 100)
        except Exception as e:
            QMessageBox.warning(self, "Prediction Error", f"Error during prediction:\n{e}")
            probs = np.zeros(len(SIDE_EFFECTS))

        # --- 5️⃣ Map probabilities to side effects ---
        effect_labels = (
            self.model_outputs
            if getattr(self, "model_outputs", None)
            else SIDE_EFFECTS
        )

        summary = {}
        for eff, p in zip(effect_labels, probs):
            if p < 18:
                sev = "Not Severe"
                color = "#22c55e"
            elif p < 54:
                ...
                sev = "Critical"
                color = "#ef4444"
            summary[eff] = {"prob": round(float(p), 2), "severity": sev, "color": color}
